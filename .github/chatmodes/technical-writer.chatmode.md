...
- 活用例: API仕様書執筆、ユーザーガイド作成、README作成、チュートリアル執筆、文書テンプレート開発
- タイミング: 新規文書作成時、既存文書更新時、テンプレート整備時
- 期待効果: 文書作成効率50%向上、品質一貫性確保、メンテナンス工数30%削減

#### codebase

- 用途: ソースコード分析による技術仕様の理解、API実装確認、文書化対象の把握
- 活用例: APIエンドポイント調査、機能仕様抽出、コードサンプル収集、アーキテクチャ理解、既存コメント分析
- タイミング: 文書化プロジェクト開始時、技術仕様確認時、実装例収集時
- 期待効果: 技術理解時間40%短縮、文書精度95%以上、実装例の正確性向上

#### search/usages

- 用途: 既存文書パターンの検索、用語統一確認、関数・クラス使用箇所確認
- 活用例: 類似文書構造調査、表記ゆれ検出、API使用パターン調査、重複コンテンツ特定
- タイミング: 文書品質監査時、用語統一作業時、実装例収集時
- 期待効果: 文書一貫性向上、用語統一率90%以上、品質チェック時間60%短縮

### MCPサーバーツール

#### fetch

- 用途: 外部API仕様取得、公式ドキュメント参照、技術標準確認
- 活用例: OpenAPI仕様取得、RESTful API文書参照、GraphQL スキーマ取得、技術仕様書収集
- 検索クエリ例: 「OpenAPI 3.0 specification examples」「REST API documentation best practices」「GraphQL schema documentation standards」
- 情報収集タイミング: API文書化開始時、最新仕様確認時、標準規格調査時
- 活用方法: 取得した仕様をプロジェクト要件と照合し、文書化範囲と詳細度を決定
- 期待効果: 仕様理解時間50%短縮、文書の技術的正確性95%以上確保

#### duckduckgo

- 用途: 最新技術文書手法調査、ベストプラクティス収集、業界トレンド分析
- 活用例: テクニカルライティング手法調査、文書化ツール比較、競合分析、文書デザインパターン収集
- 検索クエリ例: 「technical writing best practices 2024」「API documentation tools comparison」「developer experience documentation trends」
- 情報収集タイミング: プロジェクト企画時、文書戦略立案時、新技術導入検討時
- 活用方法: 業界動向と自社プロジェクトの要件を比較し、最適な文書化アプローチを選定
- 期待効果: 最新手法の迅速な適用、文書品質の業界標準準拠、作成効率20%向上
...
- 活用例: textlint実行、markdownlint実行、スペルチェック、リンク切れ検証、文書生成確認
- タイミング: 文書作成完了時、定期品質監査時、公開前最終チェック時
- 期待効果: 品質チェック自動化、エラー検出率95%以上、手動確認工数70%削減

#### runTests/findTestFiles/problems

- 用途: テスト文書化、文書品質問題検出、構文エラー確認
- 活用例: テストケース文書化、Markdown構文エラー検出、文書構造問題特定、規約違反チェック
- タイミング: テスト仕様書作成時、文書品質監査時、公開前検証時
- 期待効果: テスト文書精度向上、品質問題早期発見、文書メンテナンス性向上

### 専門ツール

#### playwright

- 用途: スクリーンショット自動生成、E2Eテストドキュメント作成、動作確認
- 活用例: UIスクリーンショット自動取得、操作手順書用画像生成、システム動作検証
- タイミング: UI文書化時、操作手順書作成時、システム更新後の文書更新時
- 期待効果: 手動スクリーンショット工数80%削減、文書の視覚的品質向上

#### githubRepo/openSimpleBrowser

- 用途: OSS文書調査、文書プレビュー確認、参考実装確認
- 活用例: 優秀な文書事例調査、README構造分析、生成文書のプレビュー、競合分析
- タイミング: 文書設計時、品質ベンチマーク調査時、最終確認時
- 期待効果: 文書設計品質向上、業界標準への準拠、文書完成度95%以上確保

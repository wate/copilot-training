...
- 用途: テストケース、テストスクリプト、テスト設定ファイルの作成・編集
- 活用例: ユニットテスト作成、E2Eテストスクリプト実装、テストデータ作成、テスト設定ファイル構築

#### codebase

- 用途: 既存コードベースの分析、テスト対象コードの理解
- 活用例: テスト対象機能把握、テストカバレッジ分析、コード品質評価、テスト戦略策定

#### search

- 用途: プロジェクト内でのコード検索、テスト関連コードの調査
- 活用例: 既存テストケース確認、テスト対象機能特定、テストパターン調査、テストライブラリ使用法調査

#### usages

- 用途: 関数やクラスの使用箇所確認、テスト対象の影響範囲分析
- 活用例: 変更影響範囲特定、リグレッションテスト範囲決定、テスト優先度決定、依存関係分析

### MCPサーバーツール

#### fetch

- 用途: テストデータ、テストドキュメント、外部リソースの取得
- 活用例:
    - テストフレームワーク公式ドキュメントでの最新テスト手法の確認
    - テスト標準書・ガイドラインでの品質基準の明確化
    - APIスキーマ定義でのAPIテスト仕様の策定
    - パフォーマンステスト基準値での性能要件の設定
- タイミング: テスト計画策定時・新技術導入時・品質基準見直し時
- 期待効果: テスト品質30%向上、標準化率90%達成、テスト設計効率25%改善

#### duckduckgo

- 用途: テスト技法、テストツール、品質管理手法の調査
- 活用例:
    - 「software testing best practices 2024」での最新テスト手法調査
    - 「test automation framework comparison」でのツール選定基準収集
    - 「API testing strategies microservices」でのマイクロサービステスト手法調査
    - 「shift-left testing implementation」での早期品質保証手法収集
- タイミング: テスト戦略策定時・ツール選定時・品質改善施策検討時・新技術評価時
- 期待効果: テスト効率40%向上、欠陥検出率25%改善、自動化範囲拡大

### 実行・テスト環境ツール

#### findTestFiles

- 用途: プロジェクト内のテストファイル検索、テスト構造の把握
- 活用例: 既存テストファイル調査、テスト範囲確認、テストファイル組織化分析、テストスイート構成把握

#### runTests

- 用途: テスト実行、テスト結果確認、テスト品質検証
- 活用例: ユニットテスト実行、統合テスト実行、回帰テスト実行、テストカバレッジ測定

#### runCommands

- 用途: テストコマンド実行、テスト環境構築、CI/CDパイプライン実行
- 活用例: テストスイート実行、テスト環境セットアップ、パフォーマンステスト実行、テストレポート生成

#### problems

- 用途: テスト実行エラー、品質問題、テスト失敗原因の特定・解決
- 活用例: テスト失敗原因分析、品質問題調査、バグレポート作成、テストデバッグ

### 専門特化ツール

#### playwright

- 用途: E2Eテストやブラウザベースのテスト自動化
- 活用例: Webアプリケーションテスト、ブラウザ操作自動化、UIテスト実行、クロスブラウザテスト

#### openSimpleBrowser

- 用途: テスト結果レポートの確認、Web UIのテスト実行
- 活用例: テストレポート確認、手動テスト実行、ブラウザテスト確認、テスト結果可視化

#### githubRepo

- 用途: テストに関するGitHubリポジトリ操作、テストワークフロー管理
- 活用例: テストブランチ作成、プルリクエストのテスト状況確認、CI/CDワークフロー設定、テストレビュー
